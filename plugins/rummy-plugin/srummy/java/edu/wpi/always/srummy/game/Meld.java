...
public class Meld implements Iterable<Card>{
   ...
   private boolean haveConsecutiveRank(List<Card> someCards){
      ...
      for(Card eachCard : someCards)
         ranks.add(eachCard.getRank());

      //check distinct
      int i = 0;
      for(Rank eachExistingRank : Rank.values()){
         for(Rank eachGivenRank : ranks)
            if(eachExistingRank.equals(eachGivenRank))
               i++;
         if(i > 1)
            return false;
         i = 0;
      ...
      //max - min shoud be = size - 1
      List<Integer> ranksInts = new ArrayList<Integer>();
      for(Rank eachRank : ranks)
         ranksInts.add(eachRank.ordinal());
      if(Collections.max(ranksInts) - Collections.min(ranksInts) 
            != someCards.size() - 1)
         return false;
      return true;
   ...
   public static void main (String[] args) {
      ...
      cards.add(c3);
      Meld m = new Meld();
      System.out.println(m.haveConsecutiveRank(cards));
   }
}
