...
public class BackChannelSchema extends SchemaImplBase {
   ...
   public BackChannelSchema (BehaviorProposalReceiver behaviorReceiver,
         BehaviorHistory behaviorHistory, ResourceMonitor resourceMonitor,
         MenuPerceptor menuPerceptor, Keyboard keyboard,
         SpeechPerceptor speechPerceptor) {
   ...
   public void onPerception (SpeechPerception perception) {
      // FaceExpressionBehavior nodBehavior = new
      // FaceExpressionBehavior(AgentFaceExpression.Nod);
      ...
      if ( swingCheckBox.choices[1] )
         nodBehavior = new FaceExpressionBehavior(
               AgentFaceExpression.Eyebrows_Up);
         ...
         if ( newState == SpeechState.Silent ) {
            if ( System.currentTimeMillis() - lastSilentAction > MIN_SILENT_ACTION_DELAY ) {
               ...
               if ( speechCounter >= SPEECH_PERIOD ) {
                  ...
                  // Lists.newArrayList(speechBehavior, nodBehavior);
                  ...
                  // Type.Before,
                  // 10));
                  // @SuppressWarnings("unused") //because voice uhum stuff
                  // unused
                  // Behavior b = new Behavior(new
                  // CompoundBehaviorWithConstraints(primitives, constraints));
                  // propose(b, m);
