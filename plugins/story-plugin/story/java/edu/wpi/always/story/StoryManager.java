...
public class StoryManager {
	...
	public OntologyStory addStory(String attitude, String name, String audience) {
		OntologyIndividual owlPerson = helper.getNamedIndividual(UUID.randomUUID().toString());
		...
		story.setTime(new Date().toString()); story.setName(name); 
		return story;
	}
	//rename: get all, find it, change
	public OntologyStory[] getStories() {
		Set<OWLNamedIndividual> owlPeople = helper.getAllOfClass(OntologyStory.STORY_CLASS);
		OntologyStory[] people = new OntologyStory[owlPeople.size()];
		int i = 0;
		for(OWLNamedIndividual owlPerson:owlPeople){
			people[i++] = new OntologyStory(ontology, new OntologyIndividual(ontology, owlPerson));
		}
		return people;
	}
}
