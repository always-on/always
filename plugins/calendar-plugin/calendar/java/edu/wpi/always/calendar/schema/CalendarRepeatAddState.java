...
abstract class CalendarRepeatAddState {
	...
	public static class EventType extends EventTypeAdjacencyPair {
		...
		public EventType(CalendarStateContext context) {
			this(context, new RepeatingCalendarEntryImpl(null, null, null, null, null, null,
					null, null, null, null, null, null));
		}
		public EventType(CalendarStateContext context, RepeatingCalendarEntry newEntry) {
			super(context);
	...
	public static class EventPerson extends EventPersonAdjacencyPair {
		...
		public EventPerson(final RepeatingCalendarEntry data, final CalendarStateContext context) {
	...
	public static class EventDay extends EventDayAdjacencyPair {
		...
		private final RepeatingCalendarEntry data;
		public EventDay(final RepeatingCalendarEntry data, final CalendarStateContext context) {
			super("Is the first time this event happens this week", context, data.getStart()!=null?CalendarUtil.getDate(data.getStart()):new LocalDate());
			this.data = data;
			if(data.getRepeatStartDate()!=null && data.getRepeatStartTime()!=null && data.getStart()!=null &&data.getRepeat()!=null && data.getRepeatEndDate()!=null)
		...
		public AdjacencyPair nextState(LocalDate date) {
			...
			data.setStart(CalendarUtil.toDateTime(date, data.getStart()!=null?CalendarUtil.getTime(data.getStart()):new LocalTime()));
	...
	public static class Frequency extends CalendarAdjacencyPairImpl {
		public Frequency(final RepeatingCalendarEntry data, final CalendarStateContext context) {
			super("How often will it take place", context);
			
			if(data.getRepeat()!=null)
				skipTo(new HowManyTimes(data, getContext()));

			for (final RepeatingCalendarEntry.Frequency frequency:RepeatingCalendarEntry.Frequency.values()) {
	...
	public static class HowManyTimes extends CalendarAdjacencyPairImpl {
		public HowManyTimes(final RepeatingCalendarEntry entry, final CalendarStateContext context) {
			...
			for (int num = 1; num <= 7; num++) {
				...
				choice(String.valueOf(num), new DialogStateTransition() {
					...
					public AdjacencyPair run() {
						...
						for(int i = 0; i<fNum-1; ++i)//one less than num because already includes start
							date = entry.getRepeat().next(date);
						entry.setRepeatEndDate(date);
						return new WhenStart(entry, new LocalTime(10, 0), getContext());
					}
				});
			}
		}
	...
	private static class WhenStart extends TimeAdjacencyPair {
		...
		public WhenStart(final RepeatingCalendarEntry entry, final LocalTime startTime, final CalendarStateContext context) {
			...
			choice("I made a mistake. Lets start over", new DialogStateTransition() {
				@Override
				public AdjacencyPair run() {
					return new Cancel(context);
				}
	...
	private static class HowLong extends HowLongAdjacencyPair {
		...
		public HowLong(RepeatingCalendarEntry data, final CalendarStateContext context) {
			super(context);
			this.data = data;
