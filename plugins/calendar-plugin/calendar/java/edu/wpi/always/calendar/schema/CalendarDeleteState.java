...
abstract class CalendarDeleteState {
	...
	private static class DeleteAll extends CalendarAdjacencyPairImpl{
		public DeleteAll(final CalendarStateContext context, final CalendarEntry entry, final LocalDate week){
	...
	private static final DateTimeFormatter CONFIRM_DELETE_DATE_FORMAT = DateTimeFormat.forPattern("");
	private static class ConfirmDelete extends CalendarAdjacencyPairImpl{
		public ConfirmDelete(final CalendarStateContext context, final CalendarEntry entry, final LocalDate week, final boolean deleteAll){
			super("Are you sure you want to delete "+entry.getDisplayTitle()+" on "+CONFIRM_DELETE_DATE_FORMAT.print(entry.getStart()), context);
