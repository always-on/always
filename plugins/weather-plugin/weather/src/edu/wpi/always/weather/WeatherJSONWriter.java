...
public class WeatherJSONWriter {
	...
	private static final DateTimeFormatter FILE_DATE_FORMAT = DateTimeFormat.forPattern("MM_dd_yyyy");
	...
	private static JsonElement toJsonStructure(WeatherReport report){
		...
		root.add("interestCities", toCityInterestJsonStructure(report.getInterestPlacesWeather()));
		root.add("friends", toPeopleInterestJsonStructure(report.getInterestPeopleWeather()));
		return root;
	}

	private static JsonElement toCityInterestJsonStructure(Map<Place, CurrentWeather> map) {
	...
	private static JsonElement toPeopleInterestJsonStructure(Map<Person, CurrentWeather> map) {
		JsonObject data = new JsonObject();
	...
	private static final DateTimeFormatter DATE_FORMAT = DateTimeFormat.forPattern("MM/dd/yy");
	...
	private static JsonElement toJsonStructure(CurrentWeather weather){
		...
		currentRoot.addProperty("weatherCondition", weather.getWeatherCondition());
