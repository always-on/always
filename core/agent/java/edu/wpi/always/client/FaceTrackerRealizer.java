...
public class FaceTrackerRealizer extends
   ...
   public void AgentFaceTracking() {
      ...
      {   
         ...
         if ( point != null ) 
         {
            ...
            if(!isSignificantMotion()) return;
            
            // Making sure the face is not a fake one based on the awkward changes in size and position.
         ...
         else
         {
            ...
            // Waiting for the lost face for a predefined period of time looking at the same direction.  
   ...
   private boolean isSignificantMotion() {
      if( Math.abs( faceProfileVector[0] - facePrevProfileVector[0] ) > faceHorizontalMovementThreshold || 
         Math.abs( faceProfileVector[1] - facePrevProfileVector[1] ) > faceVerticalMovementThreshold ) return true;
   ...
   private boolean isProportionalPosition(){
      if( Math.abs(faceProfileVector[0] - facePrevProfileVector[0]) <= faceHorizontalDisplacementThreshold &&
         Math.abs(faceProfileVector[1] - facePrevProfileVector[1]) <= faceVerticalDisplacementThreshold ) return true;
      
      return false;
   }
   
   private boolean isProportionalArea(){
      if( Math.abs(faceProfileVector[2] - facePrevProfileVector[2]) <= faceAreaThreshold ) return true;
