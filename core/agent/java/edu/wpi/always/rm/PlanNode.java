...
public class PlanNode {
   ...
   public PlanNode (Activity myActivity, RelationshipManager RM) {
      ...
      if ( RM.activityOccurrences.containsKey(myActivity.name)
         && (RM.activityOccurrences.get(myActivity.name) != null) ) {
         long elapsedMins = (new Date().getTime()
   ...
   public PlanNode (Activity myActivity, PlanNode myParent,
      ...
      long now = new Date().getTime()
         + (long) (myParent.getDuration() * Activity.DURATIONUNIT * 60000);
      /*
       * if(projectedHistory.containsKey(myActivity.name) &&
       * (projectedHistory.get(myActivity.name) != null)){ long elapsedMins =
       * Math.max((now - (long)(myActivity.duration * Activity.DURATIONUNIT *
       * 60000) - projectedHistory.get(myActivity.name).date.getTime()) / 60000,
       * 1); elapsedMins = Math.max(elapsedMins, 60); double coefficient = (1 -
       * (.9 / (Math.pow(elapsedMins/60, 0.5) )));
       * //System.out.println(coefficient); closeness *= coefficient; virtue *=
       * coefficient; if(!matchAncestor(myActivity.name)){
       * System.out.println("Mismatch:"); System.out.println("Proj. History:");
       * for(String s : projectedHistory.keySet()){ System.out.print(s + " "); }
       * System.out.println(); System.out.println("actual history:");
       * if(hasParent() && (parent.activity != null)){
       * System.out.print(parent.activity.name); if(parent.hasParent() &&
       * (parent.parent.activity != null)){
      ...
      if ( matchAncestor(myActivity.name) ) {
         ...
         // long elapsedMins = Math.max((now - (long)(myActivity.duration *
   ...
   private Hashtable<String, Occurrence> projectHistory (RelationshipManager RM) {
      return populateHistory(new Hashtable<String, Occurrence>(
            RM.activityOccurrences));
   }
   ...
   private void utilityDamping (Hashtable<String, Occurrence> history) {
      ...
      for (String activity : history.keySet()) {
         ...
         long elapsedMins = Math
               .max((new Date().getTime()
                  - (long) (getActivity().duration * Activity.DURATIONUNIT * 60000) - occ.date
                     .getTime()) / 60000, 1);
   ...
   public Hashtable<String, Occurrence> populateHistory (
         Hashtable<String, Occurrence> history) {
      if ( activity != null ) {
         long now = new Date().getTime();
   ...
   // These antisocial utility functions are obsolete. If necessary, they should
   // use personalUtility instead of directly accessing child utilities.
   public double getMaxAntisocialUtility () {
      ...
      if ( activity == null ) {
         ...
         return activity.virtue - activity.social
            + Math.max(0, activity.buildup - getExistingBuildup())
            + maxOfChildren;
      }
   }
