...
public class RelationshipManager extends Thread implements IRelationshipManager {
	...
	//Hashtable<String, Float> activities; // change to something else, using "Activity" class...
	...
	public void run(){
		...
		try{
			while(true){
				...
				// loop needed? or just update and provide new tasks when asked?
	...
	/*
	private void addActivity(String task, float salience){
		
		// Process task to calculate initial salience values
	...
	private void update(){
		...
		/*
	    //needed to update from within RM?
	     
		Enumeration<String> tasks = activities.keys();
		
		while( tasks.hasMoreElements()){
			Object task = tasks.nextElement();
	...
	private void processReports(){
		...
		for(ActivityReport r : reportQueue){
	...
	/*private TaskModel assembleModel(ArrayList<TaskModel> models){
	...
	/*private void requestTasks(){
		...
		//PlanningMessage message = new PlanningMessage();
		
		// go through potential activities, pick the best, select requisites, 
		// produce list of tasks and restrictions, then fire off the message
		
		//collab.planTasks(message);
	...
	//Main planner method.
	private void makePlans(PlanNode currentNode, ArrayList<PlanNode> leaves, double utilMargin, double limit){
	...
	private void relevanceSort(PlanNode currentNode){
		...
		if((children.size() >= 2) && (currentNode.getActivity() != null)){
			...
			ArrayList<OWLNamedIndividual> relevantTopics = relevantTopics(currentNode.getActivity().associatedTopics);

			for(PlanNode child : children){
				boolean added = false;
				
				relevanceLoop:
				for(OWLNamedIndividual childTopic : child.getActivity().associatedTopics){
					for(OWLNamedIndividual relevantTopic : relevantTopics){
						/*System.out.println("Comparison of " + childTopic.toString());
						System.out.println(" and " + relevantTopic.toString() + ": ");
						System.out.println(childTopic.compareTo(relevantTopic));*/
						if(childTopic.equals(relevantTopic)){ 
							relevant.add(child); 
							//System.out.println(currentNode.getActivity().name + " is relevant to " + child.getActivity().name);
							added = true;
							break relevanceLoop;
						}
					}
				...
				if(!added){
					...
					//System.out.println(currentNode.getActivity().name + " is NOT relevant to " + child.getActivity().name);
				}
			}
	...
	private void makePlans(PlanNode currentNode, ArrayList<PlanNode> leaves){
		...
		if(currentStage == relationshipStage.STRANGERS){
			...
			System.out.println("Relationship Stage not any appropriate value: " + currentStage.toString());
		}
		makePlans(currentNode, leaves, 0.9, limit);
	}
	
	// Checks to determine whether the given activity occurs prior to a given node in a plan.
	...
	private void assemblePlan(PlanNode currentNode, ArrayList<PlanNode> leaves, double limit){
		...
		for(Activity activity : activityQueue){
			if((currentNode.getDuration() + activity.duration <= maxDuration) 
				...
				assemblePlan(new PlanNode(activity, currentNode, this), leaves, limit);
				addedChild = true;
			}
		}
	...
	// Determines which topics are connected to any of the given array of topics by any axiom in the ontology
	//init ontology, create fail cases
	public ArrayList<OWLNamedIndividual> relevantTopics(ArrayList<OWLNamedIndividual> topics){
		...
		for(OWLNamedIndividual topic : topics){
			...
			for(OWLAxiom axiom : topic.getReferencingAxioms(ont)){
				for(OWLNamedIndividual relatedTopic : axiom.getIndividualsInSignature()){
	...
	public void relevanceTest(){
		...
		/*
		OWLOntology ont = ontology.getOWLOntology();
		Set<OWLNamedIndividual> sign = ont.getIndividualsInSignature();
		...
		/*
		System.out.println("Individuals in ontology:");
		Set<OWLNamedIndividual> individuals = ontology.getOWLOntology().getIndividualsInSignature();
		for(OWLNamedIndividual individual : individuals){
			System.out.println(individual.toString());
		}
		
		System.out.println("Active node:");
		OWLIndividual primary = (OWLIndividual)individuals.toArray()[0];
		System.out.println(primary.toString());
		ArrayList<OWLIndividual> active = new ArrayList<OWLIndividual>();
		active.add(primary);
		System.out.println("All relevant nodes:");
		for(OWLIndividual individual : relevantTopics(active)){
			System.out.println(individual.toString());
		}
		*/
	}
	...
	public PlanningMessage plan(){		
		...
		if (elapsedMins >= 1){
			currentCloseness = Math.max(((currentCloseness - baseCloseness) 
					/ (Math.pow(elapsedMins, 0.5)) ) + baseCloseness, baseCloseness) ;
		}
		...
		/*PossiblePlan planResult = makePlans(new ArrayList<Activity>(), 0.0, 0.0, 
										null, maxDuration, 0.0);
		ArrayList<Activity> activities = planResult.plan;
		*/
		...
		for(Activity activity : activityQueue){
			if (activity.priority > max){
				max = activity.priority;
		...
		if (isAccessable(topActivity)){
			collab.planTasks(new PlanningMessage(topActivity.taskModel)); 
		}
		else{
			// path req'd or retry
		}
		*/
		...
		try{ 
			...
			Element taskModel = plan.createElementNS("http://ce.org/cea-2018", "taskModel"); 
			...
			for(Activity activity: activityQueue){
				Element loadTask = plan.createElementNS("http://ce.org/cea-2018", "task");
			...
			//StreamResult result = new StreamResult(new File("Models/Today_" + nowTag + ".xml"));
			
	
			//String date = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(Calendar.getInstance().getTime());
			//StreamResult result = new StreamResult(new File("Models/Today_" + date + ".xml"));
			
			//StreamResult result = new StreamResult(new File("C:/Users/Will/today.xml"));
	...
	private void addNamespaces(Document plan, Element taskModel, PlanNode node){
		...
		if (thisActivity != null){
			Attr namespace = plan.createAttribute("xmlns:"+thisActivity.prefix);
	...
	private void writePlan(Document plan, Element supertask, Element taskModel, PlanNode node){
		
		for(PlanNode child : node.getChildren()){
			
			Element subtasks = plan.createElementNS("http://ce.org/cea-2018", "subtasks");
			...
			Element firstStep = plan.createElementNS("http://ce.org/cea-2018", "step");
			...
			if(child.hasChildren()){
				...
				Element secondStep = plan.createElementNS("http://ce.org/cea-2018", "step");
	...
	//For evaluating activities in possible plans after a hypothetical series of activities (the partialPlan)
	@SuppressWarnings("unused")
	private boolean isAccessable(Activity activity, ArrayList<Activity> partialPlan){
	...
	private boolean isAccessable(Activity activity, PlanNode node){
		double closenessRate = 1; //TODO: select appropriate rate based on collab man
...
//	public void afterInteraction(DiscoSynchronizedWrapper discoWrapper){}
	...
	// Returns an array of the activities in a path through a plan tree, specified by a leaf.
	...
	// This method is called after an interaction (surprising), analyzes the disco state to determine information about the interaction.
		...
		discoWrap.execute(new DiscoAction(){
			public void execute(Disco disco) {
				...
				/*for(int i = 0; i < stack.size(); i++){
					Segment seg = stack.get(i);
					Iterator<Object> children = seg.children();
					while(children.hasNext()){
						Object child = children.next();
						if((child instanceof Task) && true){
							history.add(((Task) child).getType());
						}
					}
				}*/
				...
				for(Task task : history){
					...
					for(Activity act : activityQueue){
						if (act.name == name){
							activityOccurrences.put(name, new Occurrence(now, act));
							stockedSocial += act.social; //modulate via usual methods?
							break;
						}
					}
				}*/
				...
				for(Task task : history){
					...
					if(activity != null){
						currentCloseness += activity.social; // TODO: modify based on damping! timing of activities important here.
					}
				}
				...
				/*	for(int i = 0; i < stack.size(); i++){
					...
					Iterator<Object> children = stack.get(i).children();
					
					System.out.println("Stack element " + i + " - " + stack.get(i).toString()+ ":");
					
					System.out.println("  Children:");
					   while(children.hasNext()){
						   Object child = children.next();
						   if(child instanceof Task){						   
							   System.out.println("    " + disco.toHistoryString(((Task) child)));
						   } else if(child instanceof Segment){
							   System.out.println("    segment - " + ((Segment) child).toString());
					   		}else{
							   System.out.println("    ???");
						   }
					   }	
					 ...
					 Segment parent = stack.get(i).getParent();
					 if(parent != null){
	...
	private void makeAntisocialPlans(PlanNode currentNode, ArrayList<PlanNode> leaves, double utilMargin, double limit){			
		...
		/*	
		ArrayList<PlanNode> checkNext = new ArrayList<PlanNode>();
		...
		while(!checkNext.isEmpty()){
			ArrayList<PlanNode> toCheck = new ArrayList<PlanNode>();	
			for(PlanNode leaf : checkNext){
				toCheck.add(leaf);
			}
			checkNext.clear();
			
			for(PlanNode leaf : toCheck){
				Activity activity = leaf.getActivity();
				if(activity.virtue - activity.social + Math.max(0, activity.buildup - leaf.getExistingBuildup()) <= 0){
					leaves.remove(leaf);
					if(leaf.hasParent()){
						PlanNode parent = leaf.getParent();
						parent.removeChild(leaf);
						if(!parent.hasChildren()){
							checkNext.add(parent);
							leaves.add(parent);
						}
					}
				}
		...
		*/
		...
		//System.out.println("max = " + maxUtil + ", accept = " + acceptableUtil);
	...
	@SuppressWarnings("unused")
	private void makeAntisocialPlans(PlanNode currentNode, ArrayList<PlanNode> leaves){
		makeAntisocialPlans(currentNode, leaves, 0.8, 100);
	}
	
	
	private void assembleAntisocialPlan(PlanNode currentNode, ArrayList<PlanNode> leaves, double limit){
		boolean addedChild = false;
		for(Activity activity : activityQueue){
			if((currentNode.getDuration() + activity.duration <= maxDuration) 
					&& !inPast(activity, currentNode)
					&& (activity.buildup <= limit)
					&& (limit == 2 || activity.name != "Introduction")){
				assembleAntisocialPlan(new PlanNode(activity, currentNode, this), leaves, limit);
				addedChild = true;
			}
		}
	...
	private void printTree(PlanNode node, int layer){
		if(node.hasChildren()){
			...
			//	System.out.println(child.getActivity().name);
				System.out.println(child.getActivity().name + " max = " + child.getMaxAntisocialUtility() + " util = " + child.getAntisocialUtility());
	...
	public void test(){
		...
		for(PlanNode leaf : leaves){
			...
			System.out.println("Plan " + count + ": " + strRel + " " + strAltRel + " " + acqRel + " " + acqAltRel + " " + comRel + " " + comAltRel);
			printPlan(planList);
			System.out.println("");
			count++;
	...
	private ArrayList<Activity> getPlanList(PlanNode node){
		...
		if(node.getActivity() != null){planList.add(node.getActivity());}
		return planList;
	}
	
	// A notion of relevance used only in the study, for comparing PLANS.
	private int getRelevance(ArrayList<Activity> listOne, ArrayList<Activity> listTwo){
		int relevance = 0;
		
		
		for(Activity activityOne : listOne){
			loop:
			for(Activity activityTwo : listTwo){
