...
public class TestPlugin extends ActivityPlugin {
   ...
   public TestPlugin () {
      ...
      try {
         ...
         activities.add(new Activity("Worcester", 1, 1, 1, 0, demoModelDoc,
               this, "test", "ns", "Talk about Worcester"));
   ...
   public void update (RelationshipManager RM) {
      Set<OWLNamedIndividual> sign = RM.getOntology().getOWLOntology()
      ...
      if ( sign.size() < 4 ) {
         ...
         for (int i = 0; i < 4; i++) {
            ...
            // System.out.println(i + ": " + activities.get((2+i)%4).name + ", "
            // + current.toString());
            iter.remove();
         }
      }
