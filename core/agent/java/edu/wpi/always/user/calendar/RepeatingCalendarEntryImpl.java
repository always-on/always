...
public class RepeatingCalendarEntryImpl extends CalendarEntryImpl implements RepeatingCalendarEntry{
	...
	public RepeatingCalendarEntryImpl(UUID id, CalendarEntryType type, Set<Person> people, Place place, DateTime instanceStart, ReadablePeriod instanceDuration,
			UUID repeatingId, LocalDate startDate, LocalDate endDate, LocalTime startTime, ReadablePeriod duration, Frequency frequency) {
	...
	public RepeatingCalendarEntry clone(){
		return new RepeatingCalendarEntryImpl(getId(), getType(), getPeople(), getPlace(), getStart(), getRepeatDuration(),
				repeatingId, startDate, endDate, startTime, duration, frequency);
	}
