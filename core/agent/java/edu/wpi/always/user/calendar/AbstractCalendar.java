...
public abstract class AbstractCalendar implements Calendar, Iterable<CalendarEntry> {
	...
	@Override
	public List<CalendarEntry> retrieve(ReadableInstant start, ReadableInstant end) {
	...
	public synchronized List<CalendarEntry> retrieve(Interval searchInterval) {
		...
		for (CalendarEntry entry : this) {
			if (searchInterval==null || searchInterval.overlaps(entry.getTime()))
	...
	public List<RepeatingCalendarEntry> retrieveByRepeatId(UUID id) {
		...
		while (entriesIterator.hasNext()) {
			...
			if ( currentEntry instanceof RepeatingCalendarEntry && ((RepeatingCalendarEntry)currentEntry).getRepeatId().equals(id)){
	...
	private boolean isRepeatingException(RepeatingCalendarEntry entry){
		if(!CalendarUtil.getTime(entry.getStart()).equals(entry.getRepeatStartTime()))
			return true;
		if(!entry.getDuration().toPeriod().equals(entry.getRepeatDuration().toPeriod()))
			return true;
	...
	private synchronized List<Conflict> update(CalendarEntry entry, boolean updateAll, boolean effectChange) {
		if(entry instanceof RepeatingCalendarEntry && updateAll){
			...
			List<RepeatingCalendarEntry> storedEntries = retrieveByRepeatId(repeatingEntry.getRepeatId());
			...
			if(storedEntries.size()>0){
				...
				for(LocalDate currentDate:oldEntry.getRepeatDateIterator()){
					boolean found = false;
					for(RepeatingCalendarEntry storedEntry:storedEntries)//all elements of stored entry should be valid 
						if(CalendarUtil.getDate(storedEntry.getStart()).equals(currentDate))
							found = true;
					if(!found)
				...
				//remove old entry
				Iterator<RepeatingCalendarEntry> storedIterator = storedEntries.iterator();
				while(storedIterator.hasNext()){
					if(storedIterator.next().getId().equals(repeatingEntry.getId()))
				...
				for(RepeatingCalendarEntry storedEntry:storedEntries)
					storedUUIDs.put(CalendarUtil.getDate(storedEntry.getStart()), storedEntry.getId());
			}
			...
			for(RepeatingCalendarEntry oldEntry:exceptionEntries)
				newEntries.add(new RepeatingCalendarEntryImpl(oldEntry.getId(), repeatingEntry.getType(), repeatingEntry.getPeople(), repeatingEntry.getPlace(),
						oldEntry.getStart(), oldEntry.getDuration(),
						repeatingEntry.getRepeatId(), repeatingEntry.getRepeatStartDate(), repeatingEntry.getRepeatEndDate(),
						repeatingEntry.getRepeatStartTime(), repeatingEntry.getRepeatDuration(), repeatingEntry.getRepeat()));
			...
			for(LocalDate date:repeatingEntry.getRepeatDateIterator()){
				if(!deletedEventDates.contains(date)){
					...
					if(id==null)
						id = UUID.randomUUID();
					newEntries.add(new RepeatingCalendarEntryImpl(id, repeatingEntry.getType(), repeatingEntry.getPeople(), repeatingEntry.getPlace(),
							CalendarUtil.toDateTime(date, repeatingEntry.getRepeatStartTime()), repeatingEntry.getRepeatDuration(),
							repeatingEntry.getRepeatId(), repeatingEntry.getRepeatStartDate(), repeatingEntry.getRepeatEndDate(),
							repeatingEntry.getRepeatStartTime(), repeatingEntry.getRepeatDuration(), repeatingEntry.getRepeat()));
				}
			}
			//make sure that the input entry has the same id as the one in the calendar (such as if you get a new event
			for(RepeatingCalendarEntry newEntry:newEntries)
				if(CalendarUtil.getDate(repeatingEntry.getStart()).equals(CalendarUtil.getDate(newEntry.getStart())))
			...
			//NOTE: any exception to the rule will persist even if is outside of the new rule
			if(effectChange){
				delete(entry, true);
				for(RepeatingCalendarEntry newEntry:newEntries){
					addEntry(newEntry);
	 ...
	 */
	@Override
	public synchronized List<Conflict> update(CalendarEntry entry, boolean updateAll) {
	 ...
	 */
	@Override
	public synchronized List<Conflict> dryUpdate(CalendarEntry entry, boolean updateAll) {
	 ...
	 */
	@Override
	public synchronized List<CalendarEntry> delete(CalendarEntry entry, boolean updateAll) {
		if(entry instanceof RepeatingCalendarEntry && updateAll){
			...
			while (entriesIterator.hasNext()) {
				...
				if ( currentEntry instanceof RepeatingCalendarEntry && ((RepeatingCalendarEntry)currentEntry).getRepeatId().equals(targetId)){
