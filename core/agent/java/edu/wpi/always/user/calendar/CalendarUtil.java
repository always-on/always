...
public class CalendarUtil {
	public static DateMidnight toMidnight(LocalDate date) {
		return new DateMidnight(date.getYear(), date.getMonthOfYear(), date.getDayOfMonth());
	}
	...
	public static DateTime toDateTime(LocalDate date, LocalTime time){
		return new DateTime(date.getYear(), date.getMonthOfYear(), date.getDayOfMonth(), time.getHourOfDay(), time.getMinuteOfHour(), 0, 0);
	}
	public static LocalDate getDate(DateTime time){
		return new LocalDate(time.getYear(), time.getMonthOfYear(), time.getDayOfMonth());
	}
