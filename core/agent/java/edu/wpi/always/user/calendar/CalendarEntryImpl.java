...
public class CalendarEntryImpl implements CalendarEntry{
	...
	public CalendarEntryImpl(CalendarEntryType type, Place place, DateTime start, ReadablePeriod duration){
		this(null, type, Collections.<Person> emptySet(), place, start, duration);
	}

	public CalendarEntryImpl(UUID id, CalendarEntryType type, Set<Person> people, Place place, DateTime start, ReadablePeriod duration){
		setId(id);
		setType(type);
	...
	public String toString(){
		return "Calendar Entry ["+getDisplayTitle()+" @ "+getPlace()+"], ["+getTime()+"] - "+getId();
	}
