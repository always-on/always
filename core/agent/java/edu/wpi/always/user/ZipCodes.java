	...
	public static class ZipCodeEntry{
		...
		ZipCodeEntry(String rawData){
			...
			for(int i = 0; i<dataStrings.length; ++i){
				dataStrings[i] = dataStrings[i].substring(1, dataStrings[i].length()-1);
			}
			zip = dataStrings[0];
			city = dataStrings[1];
			state = dataStrings[2];
			timezone = DateTimeZone.forOffsetHours(Integer.parseInt(dataStrings[5]));
		}
		...
		public String toString() {
			return "ZipCodeEntry [zip=" + zip + ", city=" + city + ", state="
					+ state + ", timezone=" + (timezone==null?timezone:timezone.getID()) + "]";
		}
		
	}
	private Map<String, ZipCodeEntry> data = new HashMap<String, ZipCodes.ZipCodeEntry>(43200);
	public ZipCodes() throws IOException{
		//data from http://www.boutell.com/zipcodes/
	   try ( Scanner s = new Scanner(
	         new BufferedInputStream(getClass().getResourceAsStream("/resources/zipcode.csv")))) {
