...
import edu.wpi.always.cm.engagement.GeneralEngagementPerception.*;
import edu.wpi.always.cm.perceptors.*;

public class GeneralEngagementPerceptorImpl implements GeneralEngagementPerceptor {
	...
	public GeneralEngagementPerceptorImpl(EmotiveFacePerceptor facePerceptor, MotionPerceptor motionPerceptor, MenuPerceptor menuPerceptor) {
		...
		synchronized (stateLock) {
			...
			if (facePerception != null) {
				if (lastFaceDistanceChange == null
					...
					lastFaceDistanceChange = new FaceDistanceTransition(facePerception.hasFace(), facePerception.isNear());
			}
			else
			...
			if (motionPerception != null) {
				if (lastMotionChange == null || lastMotionChange.hasMotion() != motionPerception.hasMovement())
					lastMotionChange = new MotionTransition(motionPerception.hasMovement());
			}
			else
			...
			if (menuPerception != null && lastTouchChange != null) {
				if (!menuPerception.selectedMenu().equals(lastTouchChange.selectedMenu())) {
					lastTouchChange = new TouchTransition(menuPerception.selectedMenu());
					hadTouch = true;
				}
			}
			else
			...
			EngagementState nextState = currentState.nextState(lastMotionChange, lastFaceDistanceChange, lastTouchChange, hadTouch, System.currentTimeMillis() - lastStateChangeTime);
	...
	protected void setState(EngagementState newState) {
		synchronized (stateLock) {
			System.out.println("Engagement Status: " + (latest != null ? latest.getState() : "") + " -> " + newState);

			// reset timing for new state
			if(lastMotionChange != null)
				lastMotionChange = new MotionTransition(lastMotionChange.hasMotion());
			
			if(lastFaceDistanceChange != null)
				lastFaceDistanceChange = new FaceDistanceTransition(lastFaceDistanceChange.hasFace(), lastFaceDistanceChange.isNear());
