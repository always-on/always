...
public class MenuTurnStateMachine implements BehaviorBuilder {
	...
	private State state;
	private DateTime waitingForResponceSince;
	...
	public Behavior build() {
		...
		if (!hasSomethingToSay(currentAdjacencyPair) && !hasChoicesForUser(currentAdjacencyPair)){
		...
		if (choicesExist)
			menuBehavior = new MenuBehavior(userChoices, currentAdjacencyPair.isTwoColumnMenu());
		...
		if (speechBehavior != null && menuBehavior != null) {
			List<PrimitiveBehavior> primitives = Lists.newArrayList(speechBehavior, menuBehavior);
			...
			b = new Behavior(new CompoundBehaviorWithConstraints(primitives, constraints));
		...
		if (menuBehavior != null && resourceMonitor.isDone(menuBehavior, lastProposal.getTimeStamp())) {
			String selectedMenu = checkMenuSelected(userChoices, lastProposal.getTimeStamp());
			if (selectedMenu != null)
				return gotoSaying(selectedMenu);
		}

		if (state == State.Hear && waitingForResponceSince.isBefore(DateTime.now().minusMillis(TIMEOUT_DELAY))) {
	...
	private String checkMenuSelected(List<String> userChoices, DateTime menuShownAt) {
	...
	private void setState(State newState) {
		if (newState == State.Hear && state == State.Say)
			waitingForResponceSince = DateTime.now();
