...
public class RealizerImpl implements Realizer, BehaviorHistory {
	...
	/*
	 * Let's talk about history locks a little: Anything that wants to change
	 * the history needs to acquire historyChangeLock. So both adding and
	 * removing from history will need this lock, but not merely reading.
	 * Anything that wants to remove stuff from history should acquire
	 * historyShrinkLock.writeLock(). Now, isDone() is implemented in a way that
	 * it will not be disturbed by adding stuff to the end of the history, but
	 * if something is removed, it will cause problems. That is why isDone()
	 * needs to get historyShrinkLock.readLock()
	 */
	...
	private class CompoundRealizerHandle implements CompoundRealizerObserver {
		...
		public void start() {
			primitiveAccess = new PrimitiveBehaviorControlDisconnectableWrapper(primitiveControl);
