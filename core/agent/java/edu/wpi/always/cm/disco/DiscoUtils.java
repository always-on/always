...
import edu.wpi.always.DiscoDocumentSet;
...
public class DiscoUtils {
	...
	public static DiscoDocumentSet loadDocumentFromResource(String resourcePath) {
		...
		String pathWithoutExtension = FilenameUtils
				.removeExtension(resourcePath);

		tryLoadPropertiesFromResource(properties, getResourceStream(pathWithoutExtension + ".properties"));
		tryLoadPropertiesFromResource(translate, getResourceStream(pathWithoutExtension + ".translate.properties"));
	...
	public static void tryLoadPropertiesFromResource(Properties properties, InputStream stream) {
		if (stream != null) {
			try {
