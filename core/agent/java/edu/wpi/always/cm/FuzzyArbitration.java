...
public class FuzzyArbitration {
		...
		for (FunctionBlock fb : fis) {
			...
			for (RuleBlock rb : fb) {
				...
				for (Rule r : rb.getRules()) {
					System.out.println("    Rule: " + r.getName() + ", support:" + r.getDegreeOfSupport());
				}
			}
		}
	...
	private void transferFieldValue (FIS fis, BehaviorMetadata other, Field fld, String prefix) {
		...
		try {
			...
			throw new FuzzyException("problem setting " + fld.getName() + " as fuzzy variable " + varName, e);
