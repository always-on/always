...
public class RummyPlugin implements Plugin {
	...
	public RummyPlugin(UIMessageDispatcher dispatcher) {
	...
	public void initInteraction() {
		Message params = Message.builder("params")
				.add("first_move", "agent")
				.build();

		Message m = Message.builder("start_plugin")
				.add("name", "rummy")
				.add(params)
				.build();
		sendToEngine(m);
	...
	public BehaviorBuilder updateInteraction(boolean lastProposalIsDone) {
		...
		if (lastMoveProposal != null && !lastProposalIsDone) {
			...
			PluginSpecificBehavior move = new PluginSpecificBehavior(this, availableMove.getValue() + "@" + availableMove.getTimeStamp(), Resource.Hand);
			...
			SyncSayBuilder b = new SyncSayBuilder(toSay, gazeAtCard, move, lookBackAtFace, new FocusRequestBehavior());
	...
	private boolean userMadeAMeldAfterMyLastMove() {
		return userMove != null && userMove.getTimeStamp().isAfter(myLastMoveTime) && isMeld(userMove.getValue());
	}
	...
	public void endInteraction() {
		Message m = Message.builder("stop_plugin")
				.add("name", "rummy")
				.build();
		sendToEngine(m);
	}
	...
	public void doAction(String actionName) {
		Message m = Message.builder("rummy.best_move")
				.build();
		sendToEngine(m);
	}
