...
public class RagCalendar implements CalendarUI{
	...
	private static final DateTimeFormatter WEEK_DATE_FORMAT = DateTimeFormat.forPattern("MMMM d, yyyy");
	private static final DateTimeFormatter DAY_DATE_FORMAT = DateTimeFormat.forPattern("EEEE, MMMM d");
	private static final DateTimeFormatter EVENT_TIME_FORMAT = DateTimeFormat.forPattern("h:mm");
	private static final DateTimeFormatter EVENT_DAY_FORMAT = DateTimeFormat.forPattern("EEE");
	private static final DateTimeFormatter EVENT_MONTH_FORMAT = DateTimeFormat.forPattern("MMMM yyyy");
	private static final DateTimeFormatter MONTH_DAY_FORMAT = DateTimeFormat.forPattern("EEEE");

	public final Calendar calendar;
	private final UIMessageDispatcher dispatcher;
	private CalendarUIListener listener;
	public RagCalendar(final Calendar calendar, UIMessageDispatcher dispatcher){
		this.calendar = calendar;
		this.dispatcher = dispatcher;

		dispatcher.registerReceiveHandler(MSG_CALENDAR_ENTRY_SELECTED, new MessageHandler() {
			@Override
			public void handleMessage(JsonObject body) {
				if(listener!=null){
					String id = body.get("id").getAsString();
					CalendarEntry entry = calendar.retrieveById(UUID.fromString(id));
					listener.entrySelected(entry);
				}
			}
		});
		dispatcher.registerReceiveHandler(MSG_CALENDAR_DAY_SELECTED, new MessageHandler() {
			@Override
			public void handleMessage(JsonObject body) {
				if(listener!=null){
	...
	private void show(CalendarUIListener listener) {
		...
		PluginUtils.startPlugin(dispatcher, PLUGIN_NAME, InstanceResuseMode.Reuse, null);
	}
	...
	public void showDay(LocalDate day, CalendarUIListener listener, boolean touchable){
		...
		Message m = Message.builder(MSG_CALENDAR_DISPLAY).add("type", "day").add("label", label).add("dayData", getDayData(dayInstant, touchable)).build();
		dispatcher.send(m);
	}
	...
	public void showWeek(LocalDate startDay, CalendarUIListener listener, boolean touchable){
		...
		String weekLabel = WEEK_DATE_FORMAT.print(startDay) +" - "+WEEK_DATE_FORMAT.print(startDay.plusDays(6));
		DateMidnight dayInstant = CalendarUtil.toMidnight(startDay);
		Message m = Message.builder(MSG_CALENDAR_DISPLAY).add("type", "week").add("weekLabel", weekLabel).add("dayData", getWeekDayData(dayInstant, touchable)).build();
		dispatcher.send(m);
	}
	...
	public int getWeeksInMonth(LocalDate startDay){
		int numWeeks = 0;
		for(LocalDate day = CalendarUtil.getFirstDayOfWeek(startDay.withDayOfMonth(1));
				day.getMonthOfYear()==startDay.getMonthOfYear() || day.isBefore(startDay);
				day = day.plusWeeks(1))
			++numWeeks;
		return numWeeks;
	}
	...
	public JsonElement getMonthDayData(LocalDate startDay, int numWeeks){
		...
		for(int r = 0; r<numWeeks; ++r){
			for(int c = 0; c<7; ++c){
				monthDayData.add(asMonthJson(r, c, CalendarUtil.toMidnight(startDay), CalendarUtil.toMidnight(day)));
				day = day.plusDays(1);
			}
		}
		return monthDayData;
	...
	private JsonElement getDayData(DateMidnight day, boolean touchable){
		...
		List<CalendarEntry> entries = calendar.retrieve(new Interval(day, day.plusDays(1)));
	...
	private JsonElement getNextWeekDayData(DateMidnight day, boolean touchable){
		JsonArray dayEvents = new JsonArray();
		List<CalendarEntry> entries = calendar.retrieve(new Interval(day.plusWeeks(1), day.plusWeeks(2)));
		for(CalendarEntry entry:entries)
	...
	private JsonElement asMonthJson(int row, int col, DateMidnight monthStart, DateMidnight day) {
		...
		jsonObject.addProperty("isThisMonth", day.getMonthOfYear()==monthStart.getMonthOfYear());
