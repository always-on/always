...
public class DianeGeneratingProgram {
	...
	public static void main(String[] args) {
		...
		program.addRegistry(new PicoRegistry(){
			...
			public void register(MutablePicoContainer container) {
				...
				container.as(Characteristics.CACHE).addComponent(ICollaborationManager.class, FakeCollaborationManager.class);
			}
		});
		...
		OntologyPeopleManager peopleHelper = program.getContainer().getComponent(OntologyPeopleManager.class);
		OntologyPlaceManager placeHelper = program.getContainer().getComponent(OntologyPlaceManager.class);
		peopleHelper.getUser().setLocation(placeHelper.getPlace("02118"));
		peopleHelper.getUser().setGender(Gender.Female);
		
		Person daughter = peopleHelper.addPerson("Ellen Lewis", Relationship.Daughter, null);
		...
		Person sister = peopleHelper.addPerson("Linda Jefferson", Relationship.Sister, null);
		sister.setLocation(placeHelper.getPlace("38120"));
		sister.setPhoneNumber("615-334-7889");
		
		
		
		Person friend1 = peopleHelper.addPerson("Harriet Jones", Relationship.Friend, Gender.Female);
		friend1.setLocation(placeHelper.getPlace("02118"));
		friend1.setPhoneNumber("617-324-0997");
		
		Person friend2 = peopleHelper.addPerson("Marion Smith", Relationship.Friend, Gender.Female);
		friend2.setLocation(placeHelper.getPlace("02124"));
		friend2.setPhoneNumber("617-238-3779");
		
		Person friend3 = peopleHelper.addPerson("Philip Morley", Relationship.Friend, Gender.Male);
		...
		listPeople(program.getContainer().getComponent(Ontology.class), peopleHelper);
	...
	public static void listPeople(Ontology ontology, OntologyPeopleManager peopleHelper) {
		for(OntologyPerson person:peopleHelper.getPeople()){
			...
			for(Relationship relationship:Relationship.values()){
				Set<OWLNamedIndividual> values = person.getIndividual().getObjectPropertyValues(relationship.name());
				if(!values.isEmpty()){
					System.out.print("\t"+relationship.name()+" = ");
					for(OWLNamedIndividual value:values){
						OntologyPerson relatedPerson = peopleHelper.getPerson(new OntologyIndividual(ontology, value));
			...
			if(birthday!=null)
				System.out.println("\tBirthday = "+birthday.getMonthOfYear()+"/"+birthday.getDayOfMonth()+"/----");
