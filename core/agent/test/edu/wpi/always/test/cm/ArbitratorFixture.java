...
public class ArbitratorFixture {
	...
	public void simpleTestWithOneSchema() {
		...
		BehaviorMetadata m = new BehaviorMetadataBuilder().specificity(1)
				.build();
		...
		SimpleCompoundBehavior expected = new SimpleCompoundBehavior(Lists.newArrayList(b1, b2));
	...
	public void whenTwoSchemasGiveConflictingProposals_ShouldPickTheHighestPriority() {
		...
		container.changeTo(new CandidateBehavior(Behavior.newInstance(b1),
				someSchema(), m1),
				new CandidateBehavior(Behavior.newInstance(b2), someSchema(),
						m2));

		arbitrator.run();

		assertEquals(1, realizer.realizedBehaviors.size());
		assertTrue(realizer.realizedBehaviors.contains(new SimpleCompoundBehavior(Lists.<PrimitiveBehavior> newArrayList(b2))));
	}
	...
	public void whenTwoSchemasGiveNonConflictingProposals_ShouldListenToBoth() {
		...
		container.changeTo(
				...
				new CandidateBehavior(Behavior.newInstance(gaze), someSchema(), m2),
				new CandidateBehavior(Behavior.NULL, someSchema(), m1) // caught
																		// a bug
																		// with
																		// this
		);
	...
	private void assertBehaviorsRealized(List<CompoundBehavior> realizedBehaviors, PrimitiveBehavior... behaviors) {
		int n = 0;

		List<PrimitiveBehavior> stillLooking = com.google.common.collect.Lists.newArrayList(behaviors);
		...
		assertEquals("More primitives realized than expected", behaviors.length, n);
		
		assertTrue(stillLooking.size() + " behaviors not realized: " + stillLooking, stillLooking.size() == 0);
	}
