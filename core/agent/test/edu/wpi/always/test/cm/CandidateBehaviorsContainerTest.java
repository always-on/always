...
public class CandidateBehaviorsContainerTest {
	...
	public void testAll_AddRecoverSingleCandidate() {
		...
		Behavior behavior = Behavior.newInstance(new GazeBehavior(new Point(1,
				1)));
	...
	public void testAll_AddTwiceFromTheSameSchema_ShouldIndluceOnlyLastOne() {
		DummySchema schema = newDummySchema();
		Behavior behavior1 = Behavior.newInstance(new GazeBehavior(new Point(1,
				1)));
		Behavior behavior2 = Behavior.newInstance(new GazeBehavior(new Point(2,
				2)));
	...
	public void testAll_ProposingBehaviorsFromDifferentThreads() {
		List<Point> gazePoints = Lists.newArrayList(new Point(1, 1), new Point(
				2, 3), new Point(10, 20));

		Random rnd = new Random();
		ScheduledExecutorService executor = Executors.newScheduledThreadPool(3);
		List<ScheduledFuture<?>> schemaTasks = Lists
				.newArrayListWithCapacity(3);

		for (int i = 0; i < gazePoints.size(); i++) {
			FakeGazeSchema schema = new FakeGazeSchema(container,
					resourceMonitor, gazePoints.get(i));
		...
		for (int i = 0; i < 7; i++) {
			...
			Iterable<Point> pointsFromBehaviors = Iterables.transform(
					candidates, new Function<CandidateBehavior, Point>() {
						public Point apply(CandidateBehavior cb) {
							GazeBehavior gazeBehavior = (GazeBehavior) ((SimpleCompoundBehavior) cb
									.getBehavior()
									.getInner()).getPrimitives().get(0);
